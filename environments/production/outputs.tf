...
output "platform_degraded_alarm" {
  description = "The single composite alarm on-call should subscribe to"
...
output "backup_vault_name" {
  description = "AWS Backup vault holding persistent volume recovery points"
  value       = module.backup.vault_name
}

output "backup_vault_arn" {
  description = "Backup vault ARN. Pass this as backup_copy_destination_vault_arn from another region."
  value       = module.backup.vault_arn
}

output "backup_selection_tags" {
  description = "Tags a volume must carry to be backed up. Apply these through the StorageClass."
  value       = module.backup.selection_tags
}
